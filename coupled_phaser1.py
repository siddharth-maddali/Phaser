...
#         self._poisson_log= tf.reduce_sum(tf.stack([r._poisson_log[1:] for r in self.recons]),axis=0)
    ...
    def phase_to_u(self,sw):
        ...
        self.shp1=phs.shape[1:]
        A = tf.linalg.matmul(tf.transpose(self.Qs),self.Qs)
        b = tf.linalg.matmul(tf.transpose(self.Qs),tf.reshape(phs,(phs.shape[0],-1)))
        self.u = tf.linalg.solve(A,b)

        norm_L = tf.reduce_sum((tf.matmul(self.Qs,self.u)-tf.reshape(phs,(phs.shape[0],-1)))**2,axis=0)
        
        norm_L = tf.reshape(norm_L,self.shp1)
        self.u = tf.reshape(self.u,(3,self.shp1[0],self.shp1[1],self.shp1[2])) 

        
        norm_L = norm_L*self.amp*small_sup
    ...
    def run_recipes(self,recipes):
        for i,recipe in enumerate(recipes):
            ...
            if i<5:
                self.center_phase()
            if i%10 == 0:
                imgs = [ai.check_get_conj_reflect(self.recons[0]._cImage,r._cImage) for r in self.recons]
                for i,r in enumerate(self.recons):
                    r._cImage = tf.Variable(imgs[i],dtype=tf.complex64)
