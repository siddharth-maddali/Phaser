...
class Mixin: 
    ...
    def __init__( self, sup, Bq ): # overridden by inheriting class
        self._support = fftshift( sup ) # the shifting is already done in the inheriting class.
        self._Bq = Bq
        self.setupDiffractionGeometry()
        return
    ...
    def setupDiffractionGeometry( self ):
        ...
        al, bt, gm = tuple( self._Bq[:,-1] ) # detector-frame direction cosines of sampling vector along rocking direction
        dq3 = gm * dq_tilde[2]
        ...
        self.__mu__     = np.exp( -1.j * 2. * np.pi * ( 
            dq3*self._grid[2] * ( 
                (al/gm)*dr1*self._grid[0] + 
                (bt/gm)*dr2*self._grid[1]
            ) 
        ) )
        self.__mustar__ = np.conj( self.__mu__ )    # one used for forward propagation, the other for backward propagation.
        self.__measure__ = dr1 * dr2 * dr3 * gm     # volume scaling measure when transforming between ortho and sheared spaces.

        self._myfftn = self.gafftn         # new geometry-aware propagators
        self._myifftn = self.gaifftn

        return
    

    def gafftn( self, arr ):
        arr_out = fftn( self.__mu__*fftn( arr, axes=[ 2 ] ), axes=[ 0, 1 ] ) * self.__measure__
        return arr_out

    def gaifftn( self, arr ):
        arr_out = ifftn( self.__mustar__ * ifftn( arr, axes=[ 0, 1 ] ), axes=[ 2 ] ) / self.__measure__
        return arr_out
